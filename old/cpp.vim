...
command! -nargs=1 RunPythonWithArgs call TermWrapper(printf('python3 %s %s', expand('%'), <f-args>))
command! -nargs=0 CompileAndRunJava call TermWrapper(printf('javac %s && java Main', expand('%')))
command! -nargs=0 CompileAndRunC call TermWrapper(printf('cc  %s && ./a.out', expand('%')))
command! -nargs=0 CompileAndRunCpp call TermWrapper(printf('g++ -std=c++11 %s && ./a.out', expand('%')))
...
autocmd FileType java nnoremap <leader>r :CompileAndRunJava<CR>
autocmd FileType python nnoremap <leader>r :RunPython<CR>
autocmd FileType python nnoremap <leader>rr :RunPythonWithArgs
...
" (i.e., xyz.cpp makesexecutable xyz, as opposed to a.out
" This C++ toolkit gives you commands to compile and/or run in different types
" of terminals for your own preference.
augroup CppToolkit
	autocmd!
...
autocmd FileType cpp nnoremap <leader>cf :CompileAndRunWithFile "input.txt"<CR>
